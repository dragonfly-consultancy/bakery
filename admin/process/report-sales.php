...
$real_end_date = "2016-12-13";*/
  ...
  $query = $db->getRows('SELECT *
    FROM invoice_hedder 
    WHERE invoice_h_date  BETWEEN CAST(? AS DATE) AND CAST(? AS DATE) AND invoice_h_status = 1 AND  invoice_h_location = ? GROUP BY invoice_h_id ORDER BY invoice_h_date DESC',[$real_start_date,$real_end_date,$_SESSION['location']]);
  ...
  foreach($data_item as $item_query) 
        { 
           ...
           $i = $i + 1;
            ...
            $invoice_item_id =  $item_query['invoice_d_item_id'];             
            $invoice_item_vat_has = $item_query['invoice_d_vat'];
            $invoice_item_price = $item_query['invoice_d_item_price'];
            $invoice_item_qty = $item_query['invoice_d_qty'];
            $invoice_item_vat = $item_query['invoice_d_vat_rate'];
            $invoice_item_detail_id = $item_query['invoice_d_id']; 
            if($invoice_item_id){
...
$i = 0;
                   ...
                   $output .= '                     <tfoot>
                                        ...
                                        <tr>
                                            ...
                                            <td  width="200px" style="text-align:right;font-weight: 600;">'.$currency.'<span class="grand_tot">'.number_format((float)$invoice_net_value,2,'.','').'</span></td>
                                                                                    </tr>
