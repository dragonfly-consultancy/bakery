...
// Fetch customers for dropdown (only customers with cart orders in the selected week)
$customers = $db->getRows("SELECT DISTINCT c.customer_id, c.customer_name 
                           ...
                           JOIN invoice_details id ON id.invoice_h_id = ih.invoice_h_id
                           WHERE ih.invoice_h_delivery_date BETWEEN ? AND ?
                             AND (ih.order_type = 'CART' OR id.is_cart_item = 1)
...
// Fetch cart orders grouped by customer, category, item, and date
$query = $db->getRows(
    "SELECT 
        c.customer_id,
        c.customer_name,
...
<head>
    <meta charset="utf-8" />
    <title>Cart Order by Customer</title>
    ...
    <style>
        ...
        /* re-using same styles as standing report */
        .report-header { display:flex; align-items:center; gap:15px; padding:15px; background:#f5f5f5; border-radius:5px; margin-bottom:20px; flex-wrap:wrap; }
        .standing-table { width:100%; border-collapse:collapse; }
        .standing-table th { text-align:center; font-weight:normal; padding:10px 8px; background:#f9f9f9; border-bottom:2px solid #ddd; font-size:11px; color:#666; }
        .standing-table td { padding:6px 8px; vertical-align:middle; border-bottom:1px solid #eee; font-size:12px; text-align:center; }
        .standing-table td.col-group, .standing-table td.col-item { text-align:left; }
        .customer-header { background:#e8f5e9; border-bottom:2px solid #4CAF50; }
        .total-row { background:#fffde7; }
    </style>
</head>
<body class="page-sidebar-closed-hide-logo page-content-white">
            ...
            <div class="page-content">
                ...
                <div class="page-bar no-print">
                    <ul class="page-breadcrumb">
                        ...
                        <li><span>Cart Order by Customer</span></li>
                    </ul>
                </div>
                ...
                <div class="standing-report">
                    ...
                    <?php if (empty($data)) { ?>
                        <div class="alert alert-info">No cart orders found for Week <?php echo $selected_week; ?>, <?php echo $selected_year; ?>.</div>
                    <?php } else { ?>
                        <table class="standing-table" id="cartTable">
                            <thead>
                                <tr>
    ...
    function exportToExcel() {
        ...
        a.href = url;
        a.download = 'cart_order_by_customer_week<?php echo $selected_week; ?>_<?php echo $selected_year; ?>.xls';
